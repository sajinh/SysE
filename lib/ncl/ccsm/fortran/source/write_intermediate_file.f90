...
subroutine write_intermediate_file(funit,hdate0, startloc0, field0, units0, desc0, &
...
! Declarations:
  ...
  units=units0
  desc=desc0
  map_source=map_source0
  nlats=nlats0

     write (FUNIT, IOSTAT=IERR) IFV

     ! WRITE the second record, common to all projections:

     write (FUNIT) HDATE, XFCST, MAP_SOURCE, FIELD, UNITS, DESC, XLVL, NX, NY, IPROJ
     if (DEBUG) then
     ...
     if (IPROJ == 0) then 
        ...
        WRITE (FUNIT) STARTLOC, STARTLAT, STARTLON, DELTALAT, DELTALON, EARTH_RADIUS

     elseif (IPROJ == 1) then 

        ! This is the Mercator projection:
        WRITE (FUNIT) STARTLOC, STARTLAT, STARTLON, DX, DY, TRUELAT1, EARTH_RADIUS

     elseif (IPROJ == 3) then

        ! This is the Lambert Conformal projection:
        WRITE (FUNIT) STARTLOC, STARTLAT, STARTLON, DX, DY, XLONC, TRUELAT1, TRUELAT2, EARTH_RADIUS
        

     elseif (IPROJ == 4) then

        ! Gaussian projection                         
        WRITE (FUNIT) STARTLOC, STARTLAT, STARTLON, NLATS, DELTALON, EARTH_RADIUS
        
     elseif (IPROJ == 5) then

        ! This is the Polar Stereographic projection:
        WRITE (FUNIT) STARTLOC, STARTLAT, STARTLON, DX, DY, XLONC, TRUELAT1, EARTH_RADIUS

     endif

     
     WRITE (FUNIT) IS_WIND_EARTH_REL
